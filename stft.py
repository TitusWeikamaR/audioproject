...
def stft(x, frame_size, hop_size=None, window=None, N=None, only_positive_freqs=True):
    ...
    # compute the stft
    X = np.array([np.fft.fft(window * x[i:i + frame_size], N)
                  for i in range(0, len(x) - frame_size + 1, hop_size)]).T
...
def istft(X, frame_size, hop_size=None, window=None, only_positive_freqs=True):
    ...
    for n, i in enumerate(range(0, len(x) - frame_size + 1, hop_size)):
        x[i:i + frame_size] += window * \
            np.real(np.fft.ifft(X[:, n], n=frame_size))
...
if __name__ == '__main__':
    ...
    sr, d = scipy.io.wavfile.read('../project.wav')
    ...
    scipy.io.wavfile.write('../project01_recons.wav', sr, x)
